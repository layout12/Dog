...
(function(o,K){
	function Ia(a,b){
		...
		if(a.date){
			if(!a.start)
				a.start=a.date;delete a.date
			}
			a._start=q(a.start=ib(a.start));
			a.end=ib(a.end);
			if(a.end&&a.end<=a.start)
				a.end=null;a._end=a.end?q(a.end):null;
				if(a.allDay===K)
	...
	function Wa(a,b,f,c){
		function g(m,e,j,r){
			N=m;L=e;ca=b.theme?"ui":"fc";O=b.weekends?0:1;ua=b.firstDay;
			if(ra=b.isRTL){
				ka=-1;R=L-1
				}else{
					ka=1;R=0
			...
			if(N<e)U.find("tr:gt("+(N-1)+")").remove();
				...
				for(e=e;e<N;e++){
					...
					for(E=0;E<L;E++){
						m+="<td class='fc-"+za[B.getDay()]+" "+ca+"-state-default fc-new fc-day"+(e*L+E)+(E==R?" fc-leftmost":"")+"'>"+(r?"<div class='fc-day-number'></div>":"")+"<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></td>";C(B,1);O&&da(B)}m+="</tr>"}U.append(m)}fa(U.find("td.fc-new").removeClass("fc-new"));
